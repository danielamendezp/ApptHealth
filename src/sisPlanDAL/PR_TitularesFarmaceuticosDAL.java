...
public class PR_TitularesFarmaceuticosDAL  extends BaseDAL {

    public PR_TitularesFarmaceuticosDAL(){
        //Rutas para los archivos de datos
        this.rutaArchivoDatos = ".\\src\\sisPlanDatos\\TITULARES_FARMACEUTICAS.dat";
        this.numeroColumnaLlavePrimaria = 0;
        this.oManejadorArchivosDAL = new ManejadorArchivosDAL();        
    }

     public TitularFarmaceutica buscarTitularFarmaceutica(int pIdentificador) {
        TitularFarmaceutica oTitularFarmaceutica;
        ...
        oTitularFarmaceutica = new TitularFarmaceutica(Integer.parseInt(vDatosRegistro[0]), 
                                    vDatosRegistro[1]);
        return oTitularFarmaceutica;
    }
    
    /**
     * Obtener la lista completa de las TitularFarmaceuticas
     * @return Objeto ArrayList con todos los objetos departamentos
     */
    
    public List<TitularFarmaceutica> getListaTitularFarmaceutica() {
        // Definición de variable entidad
        TitularFarmaceutica oTitularFarmaceutica= null;

        // Definir objeto ArrayList para cargar los objetos
        List<TitularFarmaceutica> oArrayListTitularFarmaceuticas = new ArrayList<>();
        ...
        for (String vRegistro : oListaRegistros) {
            ...
            if (vDatosRegistro.length == 2) {
                try {
                    // Crear la instancia de Departamento
                    oTitularFarmaceutica = new TitularFarmaceutica(Integer.parseInt(vDatosRegistro[0]), vDatosRegistro[1]);

                    // Agregar la instancia a la colección
                    oArrayListTitularFarmaceuticas.add(oTitularFarmaceutica);
        ...
        return oArrayListTitularFarmaceuticas;
    }
}
